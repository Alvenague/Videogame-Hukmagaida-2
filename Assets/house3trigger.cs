...
public class house3trigger : MonoBehaviour {
	...
	void OnTriggerEnter(Collider other) {
		...
		if (firstTime) {
			...
			missionTxt.guiText.text = "I've just found house3.";
			StartCoroutine(WaitAndPrint(3.0F));
			
			GameObject newTracker = GameObject.Find("House32");
			tracker.GetComponent<ObjectLabel>().target = newTracker.transform;
			GameObject trigger = GameObject.Find("house32Trigger");
